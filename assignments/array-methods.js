...
let fullName = [];
runners.forEach((current, index, arr) => {
    fullName[index] = {'first_name': current.first_name, 'last_name': current.last_name};
});
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [...runners].map(current => {
    let copied = Object.create(current);
    copied.first_name = copied.first_name.toUpperCase();
    return copied;
});
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce((accumulator, runner) => accumulator + runner.donation, 0);
...
// Problem 1
// Create a list of email addresses to send the results to
let emailAddresses = runners.map((current) => current.email);
console.log(emailAddresses);

// Problem 2
// get the biggest donator
let biggestDonator = runners.reduce((accumulator, runner) => accumulator.donation < runner.donation ? runner : accumulator);
console.log(biggestDonator); //should hold the runner object of the biggest donator

// Problem 3
// find everyone with shirt size of at least 1X
let bigRunners = [...runners].filter(runner => {
    return runner.shirt_size.match(/^\d?XL$/); // match() returns an array if any matches are found, which evaluates to true
});